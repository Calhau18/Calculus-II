...
\begin{itemize}
	\item \textbf{Massa}:\\
Seja $\sigma: D \to \mathbb{R}$ uma função que representa a densidade de um sólido representado por $D \subset \mathbb{R}^n$. Então, a massa do sólido é dada por
$$
\int_D \sigma
$$
Note-se que se $\sigma$ é constante ($\sigma(x) = \rho, \forall x \in D$), temos que $M = V \cdot \rho$.
	\item \textbf{Centro de Massa}:\\
Recuperando a função densidade $\sigma: D \to \mathbb{R}$ do sólido representado por $D$, temos que o centro de massa do sólido é dado por
$$
...
Seja $L$ uma linha reta e $d_L(x)$ a distância do ponto $x \in \mathbb{R}^n$ à linha $L$. O momento de inércia do conjunto $D$ relativo à reta $L$, é dado por
$$
